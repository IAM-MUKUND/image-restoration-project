...
gt_dir = data_root / "GT"
tta8_pred_dir = project_dir / "artifacts/remote-runs/daf-final-submission/predictions"
output_path = project_dir / "artifacts/remote-runs/daf_1view_8view_vs_gt_comparison.png"

# Load pre-trained DAF-Restormer checkpoint for 1-view direct inference
...
print(f"Generating 4-column comparison for samples: {sample_names}")
...
for row, name in enumerate(sample_names):
    ...
    with torch.no_grad():
        restored_1view_tensor = model(noisy_tensor)
    restored_1view_arr = restored_1view_tensor.clamp(0.0, 1.0).squeeze().cpu().numpy()
    
    # Load DAF-Restormer 8-View TTA prediction if available, or compute fallback 8-view TTA
    tta8_file = tta8_pred_dir / name
    if tta8_file.exists():
        restored_8view_arr = np.load(tta8_file, allow_pickle=False).astype(np.float32)
    else:
        # Compute 8-view TTA on the fly
        preds = []
        for rot in range(4):
            for flip in [False, True]:
                inp = torch.rot90(noisy_tensor, k=rot, dims=[-2, -1])
                if flip:
                ...
                preds.append(out)
        restored_8view_arr = torch.stack(preds).mean(dim=0).clamp(0.0, 1.0).squeeze().cpu().numpy()
    ...
    axes[row, 1].set_title(f"DAF 1-View (256x256)\n{psnr_1view:.2f} dB | ~35.8 ms", fontsize=11, fontweight="bold", color="#1f77b4")
    axes[row, 1].axis("off")

    # Col 3: DAF-Restormer 8-View TTA (256x256)
    axes[row, 2].imshow(restored_8view_arr, cmap="gray", vmin=0, vmax=1)
    axes[row, 2].set_title(f"DAF 8-View TTA (256x256)\n{psnr_8view:.2f} dB | ~264.7 ms", fontsize=11, fontweight="bold", color="#ff7f0e")
    axes[row, 2].axis("off")
...
plt.suptitle("Detailed Visual Comparison: Noisy LR vs DAF 1-View vs DAF 8-View TTA vs Ground Truth GT", fontsize=14, y=0.99, fontweight="bold")
plt.tight_layout()
plt.savefig(output_path, dpi=200, bbox_inches="tight")
plt.close(fig)

print(f"Successfully generated 4-column comparison image at {output_path}")
