...
icnr_checkpoint_path = project_dir / "artifacts/remote-runs/daf-curriculum-stage2/daf_restormer/checkpoints/best.pt"

output_path = project_dir / "artifacts/remote-runs/daf_icnr_clean_loss_comparison.png"
...
# Load ICNR-Initialized DAF model
icnr_model = build_model("daf_restormer").to(device)
if icnr_checkpoint_path.exists():
    ckpt = torch.load(icnr_checkpoint_path, map_location=device, weights_only=False)
    state_dict = ckpt["model"] if isinstance(ckpt, dict) and "model" in ckpt else ckpt
    icnr_model.load_state_dict(state_dict, strict=False)
icnr_model.eval()
...
for row, name in enumerate(sample_names):
    ...
    with torch.no_grad():
        ...
        icnr_out = icnr_model(noisy_tensor).clamp(0.0, 1.0).squeeze().cpu().numpy()
    
    mse_orig = float(np.mean((orig_out - gt_arr) ** 2))
    psnr_orig = -10.0 * np.log10(max(mse_orig, 1e-12))

    mse_icnr = float(np.mean((icnr_out - gt_arr) ** 2))
    psnr_icnr = -10.0 * np.log10(max(mse_icnr, 1e-12))
    ...
    # Col 2: Original DAF 1-View (With PixelShuffle checkerboard grain)
    axes[row, 1].imshow(orig_out, cmap="gray", vmin=0, vmax=1)
    axes[row, 1].set_title(f"Original DAF 1-View\nPSNR: {psnr_orig:.2f} dB (Checkerboard Grain)", fontsize=10, fontweight="bold", color="#d62728")
    axes[row, 1].axis("off")

    # Col 3: ICNR-Initialized DAF 1-View (Clean Loss)
    axes[row, 2].imshow(icnr_out, cmap="gray", vmin=0, vmax=1)
    axes[row, 2].set_title(f"ICNR-Init DAF (Clean Loss)\nPSNR: {psnr_icnr:.2f} dB (Grain-Free)", fontsize=10, fontweight="bold", color="#1f77b4")
    axes[row, 2].axis("off")
...
plt.suptitle("PixelShuffle Grain Fix Comparison: Original DAF vs ICNR-Init DAF (Clean Loss) vs Ground Truth GT", fontsize=13, y=0.99, fontweight="bold")
plt.tight_layout()
